...
La Lista Doblemente Enlazada tiene una buena eficiencia y estable en sus operaciones clave: para las operaciones de gestión de inventario (añadir o remover elementos en los extremos), la complejidad se mantiene consistentemente en tiempo constante, O(1), tanto en el mejor caso, promedio como en el peor caso. Esto se debe a que la estructura utiliza punteros directos a head y tail, requiriendo solo un número fijo de reasignaciones de punteros (next y prev). Sin embargo, la complejidad se mueve a tiempo lineal, O(N) cuando la operación requiere, inevitablemente, recorrer todos los nodos de la lista, como sucede con funciones de consulta completa (mostrar()), (clear()) por lo tanto, el peor caso individual de la estructura, cuando se considera cualquier posible operación, está determinado por O(N) de recorrido total.
